...
> 基于 V2.3.0 继承能力与 V3.0.0 开发代码的产品化功能设计
> 更新日期：2026-07-19
...
3. **安全降级**：原生发现失败时先回退到缓存节点和经握手校验的手动地址；二维码作为 P1 扩展。
...
1. Tauri 首先注册单实例插件，再注册深链、通知、自启、Updater、Process 和 Dialog 插件。
2. 加载 `frontend/dist` 或开发服务器。
3. 前端通过 Native Bridge 读取运行时与客户端版本。
4. 从应用数据目录恢复最多 32 个节点缓存，并读取当前选中节点。
5. 进入节点探测和认证流程。
...
**验收**：macOS 环境可启动、发现/手动选择节点、登录并执行 V2.3 主链路；开发模式支持热更新，Windows/Linux 进入 smoke build 矩阵。
...
当前节点状态、免打扰和诊断快捷入口属于后续增强，不作为本轮已实现能力。

**关闭窗口策略**：

- 点击窗口关闭按钮默认隐藏到托盘；
- 只有托盘“退出”或显式退出命令才终止进程；
...
```text
lanchat://node?origin=<encoded-http(s)-origin>
lanchat://room/<roomCode>?node=<optional-encoded-origin>
lanchat://conversation/<conversationId>?node=<optional-encoded-origin>
lanchat://broadcast/<broadcastId>?node=<optional-encoded-origin>
```

深链只能表达导航目标，不直接携带 Access Token、Refresh Token、密码或签名文件 URL。解析器拒绝未知目标、凭据、片段、重复/未知参数、额外路径段和非 HTTP(S) 节点地址；冷启动目标先暂存，第二实例目标转交主实例。
...
二维码和管理员预置列表属于 P1，接入时必须复用深链/手动地址的白名单与握手校验，不得绕过验证。

### 6.5 安全规则

- 发现结果不等于可信节点，必须执行 HTTPS/证书或明确内网安全提示；
- 节点握手不得返回数据库、Redis、MinIO 凭据；
- 地址切换前清理旧节点的 Access Token，并按“节点 ID + Origin + 用户”隔离本地缓存；
...
- 普通节点缓存：Tauri 应用数据目录中的版本化 JSON，限制条数并使用原子替换；
- 大量业务缓存：IndexedDB 或经过评估的本地数据库；
- Access Token：前端 `sessionStorage`，按选中节点切换和清理；
- Refresh Token：Rust `reqwest` 按 Origin 隔离的内存 Cookie Jar，不向 JavaScript 返回；
- 当前不提供跨应用重启的 Refresh Token 持久化；若后续需要，必须使用系统安全存储或 Stronghold 并经过威胁建模。
...
| `desktop-build.yml` | PR / push / 手动 | Windows、macOS、Linux Rust 检查、测试、桌面前端和无签名 smoke build |
| `e2e.yml` | PR / 手动 / 夜间 | Docker 双实例消息与浏览器断网恢复；可选自托管 mDNS 前置门禁 |
| `release-desktop.yml` | tag / 手动 | 三平台签名草稿 Release 与 updater JSON；缺少密钥直接失败 |
| `server-release.yml` | tag / 手动 | 服务端镜像、GHCR、Compose 发布包和校验和 |
| Android workflow | P1 | Web build、Cap sync、Gradle test/build、签名 AAB，当前未实现 |

自托管 mDNS job 当前只检查 runner 具有多播网卡，不代表已经覆盖真实服务发现；后续必须在该 runner 上运行原生发现测试。
...
当前 Rust 单测覆盖安全深链、手动地址规范化、节点 ID、发现地址过滤、缓存去重/裁剪/排序、枚举契约和更新状态分类等纯逻辑；系统托盘、通知、自启、真实多播和签名更新仍需在对应操作系统执行验收。
...
当前 Playwright/API/WebSocket E2E 已实现“跨实例消息只落库一次”和“浏览器断网文本发件箱恢复后只投递一次”。Presence、WebRTC、Redis 中断、SYNC 缺口与上传恢复仍是待补场景，不能视为已验证。
