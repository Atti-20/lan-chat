...
<template>
  <section class="workspace-welcome" :data-section="section" aria-live="polite">
    <div class="welcome-signal" aria-hidden="true">
      ...
      <span class="signal-core"><UiIcon :name="content.icon" :size="34" /></span>
      <i /><i /><i />
    </div>
    <p>{{ content.kicker }}</p>
    <h2>{{ content.title }}</h2>
...
.welcome-signal { position: relative; display: grid; width: 116px; height: 116px; margin-bottom: 22px; place-items: center; }
...
.signal-core { display: grid; width: 76px; height: 76px; place-items: center; border-radius: 24px; color: var(--blue); background: var(--fill); box-shadow: inset 0 1px 0 var(--highlight-soft); }
.welcome-signal i { position: absolute; width: 7px; height: 7px; border: 2px solid var(--surface); border-radius: 50%; background: var(--blue); }
.welcome-signal i:nth-of-type(1) { top: 9px; right: 18px; }
.welcome-signal i:nth-of-type(2) { bottom: 12px; left: 13px; background: var(--green); }
.welcome-signal i:nth-of-type(3) { right: 4px; bottom: 32px; background: var(--violet); }
.workspace-welcome > p { margin: 0 0 7px; color: var(--blue); font-size: 10px; font-weight: 700; letter-spacing: .08em; }
.workspace-welcome > h2 { margin: 0; font-size: 21px; letter-spacing: -.04em; }
.workspace-welcome > span { max-width: 390px; margin: 10px 0 20px; color: var(--ink-soft); font-size: 12px; line-height: 1.75; }
...
@media (max-width: 760px) {
  .workspace-welcome { padding: 26px 22px 104px; }
}
</style>
