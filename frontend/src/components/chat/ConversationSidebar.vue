...
.action-menu button:focus-visible { color: var(--accent-text); background: var(--hover); }
...
.sidebar-search { display: flex; min-height: 42px; padding: 0 12px; align-items: center; gap: var(--space-2); border: 1px solid rgba(144,169,194,.17); }
...
.sidebar-search kbd { padding: 3px 5px; border: 1px solid rgba(138,163,188,.2); border-radius: 5px; color: #8293a5; font-family: inherit; font-size: var(--font-micro); }
...
.conversation-item { display: flex; width: 100%; min-height: 74px; align-items: center; border: 1px solid transparent; text-align: left; background: transparent; cursor: pointer; transition: 200ms var(--ease-liquid); }
...
.conversation-line { display: flex; min-width: 0; align-items: center; gap: var(--space-2); }
.conversation-line strong { overflow: hidden; flex: 1; text-overflow: ellipsis; white-space: nowrap; }
.conversation-line time { color: var(--ink-faint); }
...
.conversation-preview .unread-badge { display: inline-flex; width: 20px; height: 20px; padding: 0; align-items: center; justify-content: center; flex: 0 0 20px; box-sizing: border-box; border-radius: 50%; color: white; font-size: 9px; font-weight: 750; font-style: normal; font-variant-numeric: tabular-nums; letter-spacing: -.06em; line-height: 1; white-space: nowrap; background: var(--coral); }
.conversation-preview .pending { min-width: 17px; padding: 1px 5px; border-radius: var(--radius-pill); color: var(--accent-text); font-size: var(--font-micro); font-style: normal; text-align: center; background: rgba(0,122,255,.1); }
.conversation-pin { display: grid; width: 20px; height: 20px; margin-left: auto; place-items: center; color: var(--accent-text); }
.conversation-pin .ui-icon { width: 15px; height: 15px; }
.request-section { margin-bottom: 4px; }
.request-section-title { color: var(--ink-faint); font-size: var(--font-micro); font-weight: 700; }
...
.request-copy strong { font-size: var(--font-body-sm); }
.request-copy p { margin: 3px 0; overflow: hidden; color: var(--ink-soft); font-size: var(--font-micro); text-overflow: ellipsis; white-space: nowrap; }
.request-copy span { color: var(--ink-faint); font-size: var(--font-micro); }
.request-actions { display: flex; grid-column: 1 / -1; gap: var(--space-2); }
.request-actions button { min-height: 32px; padding: 0 13px; border: 0; border-radius: var(--radius-control); color: var(--on-surface); font-size: var(--font-micro); font-weight: 700; background: var(--fill); cursor: pointer; }
.request-actions .accept { color: white; background: var(--action-bg); box-shadow: 0 6px 13px rgba(10,132,255,.2); }
.empty-list { display: grid; min-height: 260px; flex: 1; padding: 30px; place-items: center; align-content: center; text-align: center; }
.empty-list > span { display: grid; width: 52px; height: 52px; margin-bottom: 15px; place-items: center; border: 1px solid rgba(255,255,255,.8); border-radius: 19px; color: var(--accent-text); font-size: 23px; background: rgba(255,255,255,.48); box-shadow: inset 0 1px 0 #fff; }
.empty-list strong { font-size: var(--font-body); }
.empty-list p { margin: 6px 0 0; color: var(--ink-soft); font-size: var(--font-micro); line-height: 1.5; }
.search-state { margin: 0; padding: 28px 16px; color: var(--ink-faint); font-size: var(--font-caption); line-height: 1.5; text-align: center; }
.search-state--error { color: var(--danger); }
...
.sidebar-header h1 { font-size: var(--font-page-title); font-weight: 700; letter-spacing: -0.03em; }
...
.mini-button:hover { color: var(--accent-text); background: var(--button-hover); }
.sidebar-search {
  min-height: 38px;
  margin: 0 12px 10px;
  border: 0;
...
.conversation-line strong { font-size: var(--font-body); }
.conversation-line time { font-size: var(--font-caption); }
.conversation-preview { color: var(--ink-faint); font-size: var(--font-caption); }
...
.empty-icon .ui-icon { width: 24px; color: var(--accent-text); }
.sidebar-loading span { height: 54px; border-radius: var(--radius-control); background-color: var(--fill); }
