...
const tokenUrl = new URL('../../../packages/design-tokens/tokens.css', import.meta.url)
const mainCss = await readFile(tokenUrl, 'utf8')
...
function resolve(values, name, stack = []) {
  assert.ok(!stack.includes(name), `circular CSS token: ${[...stack, name].join(' -> ')}`)
  assert.ok(name in values, `missing CSS token: ${name}`)
  return values[name].replace(/var\((--[\w-]+)\)/g, (_, target) => resolve(values, target, [...stack, name]))
}

function luminance(hex) {
  assert.match(hex, /^#[\da-f]{6}$/i, `expected an opaque contrast color: ${hex}`)
...
test('semantic text and filled actions remain readable in both themes', () => {
  for (const [theme, values] of Object.entries({ light, dark })) {
    ...
    for (const [fg, bg] of pairs) {
      const a = luminance(resolve(values, fg)), b = luminance(resolve(values, bg))
...
test('semantic message colors and legacy aliases agree in both themes', () => {
  for (const values of [light, dark]) {
    assert.equal(resolve(values, '--ink'), resolve(values, '--mx-color-text-primary'))
    assert.equal(resolve(values, '--mx-color-message-own'), resolve(values, '--action-bg'))
    assert.equal(resolve(values, '--mx-color-message-peer'), resolve(values, '--fill'))
    assert.equal(resolve(values, '--mx-typography-body-size'), '0.875rem')
  }
})
...
test('component CSS cannot silently reference an undefined design token', async () => {
  const sheets = await Promise.all([tokenUrl, ...await sourceFiles(source)].map(async (url) => {
