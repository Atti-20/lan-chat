...
test('a delayed notification event cannot consume a newer message or owner route', () => {
  const store = new PendingNavigationStore(memoryStorage())
  const old = {kind: 'conversation', value: 'group:21', notification: true, notificationOwner: 'node|7', messageId: 'first'}
  const next = {...old, messageId: 'second'}
  store.store(next)
  assert.equal(store.claim(old), null)
  assert.equal(store.claim({...next, notificationOwner: 'node|8'}), null)
  assert.equal(store.claim(next)?.messageId, 'second')
})
