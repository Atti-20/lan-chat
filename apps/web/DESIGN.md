...
版本：2026-09-08。基准：当前 `apps/web/` 的 Vue 界面。适用于浏览器、Tauri 桌面壳及共享 Vue 的移动壳。

本文件是 [跨端设计体系](../../docs/design/README.md) 的 Vue 实现细则。跨端组件状态在 [components.json](../../packages/design-tokens/components.json)。
...
| 职责 | 唯一入口 |
| --- | --- |
| 基础控件、焦点与辅助偏好 | [`src/assets/main.css`](src/assets/main.css) |
| 颜色、字体、间距、圆角、主题材质变量 | 修改 [`tokens.json`](../../packages/design-tokens/tokens.json)，生成 [`tokens.css`](../../packages/design-tokens/tokens.css) |
...
2026-09-08：中立 `tokens.json` 统一生成 CSS 与 Dart，保留现有 CSS 视觉值。控件与布局仍在主 CSS 和组件，不能手改生成 CSS。

不新增第二套全局样式入口或 UI 框架。新增界面先复用主 CSS 的变量和控件；专属布局保留在组件内。修改规范中的基础值时，同时修改中立 token、生成物及受影响的页面，并进行渲染复查。
...
字号优先使用 rem 变量，不修改根字号来抵消用户缩放。粗指针设备上的 input / textarea / select 至少 16px，避免 iOS 聚焦时放大页面。输入、下拉框、按钮统一继承字体。 消息输入框为空时，占位文字单行省略，避免窄屏换行后被裁切；输入实际内容后恢复多行换行与自适应高度。
...
- 命令：`npm --prefix apps/web test`、`npm --prefix apps/web run typecheck`、`npm --prefix apps/web run build`。
...
## MX-A04 设计基础（2026-09-09）

当前视觉为基线。Token source 升级为 Primitive / Semantic / 必要 Component 三层，保留全部兼容 alias；新代表组件样式使用 `--mx-*`。生成方式与精确类型规则见 [design-tokens README](../../packages/design-tokens/README.md)，状态以 [六类规范](../../docs/design/component-specs/README.md) 为准。Token 层不负责主题偏好或业务状态；不把规范中的未实现状态当成功能。

已接入的生产组件：UserAvatar、ConversationSidebar、MessageThread；公共 Button/Input 类同步使用语义 token。局部硬编码/小按钮/消息选择等 [设计债](../../docs/design/debt/README.md) 保留。使用 `./tooling/verify design`、`./tooling/verify web` 和 [浏览器验证](../../docs/design/validation.md)，不手改 CSS/Dart 生成物。
