...
.log-entry--warn {
  ...
  background: linear-gradient(100deg, color-mix(in srgb, var(--warning) 7%, var(--surface-raise)), var(--surface-raise) 52%);
}
.log-entry--warn::before { width: 4px; background: var(--warning); opacity: 1; }
.log-entry--error {
...
.entry-header { display: flex; min-width: 0; align-items: center; gap: 9px; color: var(--ink-faint); font-family: var(--font-mono); font-size: var(--font-micro); }
.level-badge { padding: 3px 7px; border-radius: var(--radius-pill); color: var(--ink-soft); font-family: inherit; font-weight: 800; background: var(--fill); }
.log-entry--info .level-badge { color: var(--accent-text); background: color-mix(in srgb, var(--blue) 10%, transparent); }
.log-entry--warn .level-badge { color: #b65f00; background: color-mix(in srgb, var(--warning) 13%, transparent); }
.log-entry--error .level-badge { color: var(--danger); background: color-mix(in srgb, var(--coral) 13%, transparent); }
...
.entry-message { margin: 10px 0 0; color: var(--ink); font-family: var(--font-mono); font-size: var(--font-micro); line-height: 1.65; overflow-wrap: anywhere; white-space: pre-wrap; }
.entry-explanation { display: grid; padding: 10px 12px; margin-top: 11px; grid-template-columns: 64px minmax(0, 1fr); align-items: start; gap: 9px; border-radius: var(--radius-sm); color: var(--ink-soft); background: color-mix(in srgb, var(--warning) 8%, var(--surface)); }
.log-entry--error .entry-explanation { background: color-mix(in srgb, var(--coral) 9%, var(--surface)); }
.entry-explanation strong { color: var(--warning); font-size: var(--font-caption); }
.log-entry--error .entry-explanation strong { color: var(--danger); }
.entry-explanation p { margin: 0; font-size: var(--font-caption); line-height: 1.65; }
.entry-details { margin-top: 9px; }
.entry-details summary { width: fit-content; color: var(--accent-text); font-size: var(--font-micro); font-weight: 700; cursor: pointer; }
.entry-details pre { max-height: 280px; padding: 11px; margin: 8px 0 0; overflow: auto; border: 1px solid var(--separator); border-radius: var(--radius-sm); color: var(--ink-soft); font-family: var(--font-mono); font-size: var(--font-micro); line-height: 1.55; background: color-mix(in srgb, var(--ink) 4%, var(--surface)); white-space: pre-wrap; overflow-wrap: anywhere; }
.entry-footer { margin-top: 9px; color: var(--ink-faint); font-family: var(--font-mono); font-size: var(--font-micro); }
...
.stream-state .ui-icon { margin-bottom: 12px; color: var(--accent-text); }
.stream-state strong { color: var(--ink); font-size: var(--font-body-sm); }
.stream-state p { max-width: 420px; margin: 7px 0 0; font-size: var(--font-caption); line-height: 1.65; }
.stream-state button { min-height: 34px; padding: 0 12px; margin-top: 14px; border: 0; border-radius: var(--radius-sm); color: var(--accent-text); font: inherit; font-size: var(--font-caption); font-weight: 700; background: var(--active); cursor: pointer; }
.stream-state--error .ui-icon { color: var(--danger); }
.stream-state--compact { min-height: 240px; }
.state-spinner { width: 24px; height: 24px; margin-bottom: 13px; border: 2px solid color-mix(in srgb, var(--blue) 18%, transparent); border-top-color: var(--accent-text); border-radius: 50%; animation: log-spin .8s linear infinite; }
...
@media (max-width: 760px) {
  ...
  .entry-explanation { grid-template-columns: 1fr; gap: var(--space-1); }
}
