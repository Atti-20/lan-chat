...
import { computed, nextTick, onMounted, onBeforeUnmount, shallowRef, useTemplateRef, watch } from 'vue'
import type { ChatMessage, RecoveryTerminal, Conversation, GroupMember, MentionReadReceipt, User } from '../../types'
...
const displayRows = computed(() => [
  ...props.messages.map(message=>({key:message.messageId,sequence:message.sequence || Number.MAX_SAFE_INTEGER,message,terminal:undefined as RecoveryTerminal | undefined})),
  ...props.recoveryTerminals.map(terminal=>({key:terminal.messageId,sequence:terminal.sequence,message:undefined as ChatMessage | undefined,terminal})),
].sort((a,b)=>props.recoveryTerminals.length ? a.sequence-b.sequence : 0))
const terminalLabel = (state: RecoveryTerminal['state']) => state==='RECALLED' ? '这条消息已撤回' : state==='BURNED' ? '这条消息已焚毁' : '这条消息已不可用'
...
onMounted(()=>{readVisibilityTimer=window.setInterval(()=>{
  ...
  if(viewport && document.visibilityState==='visible' && !props.recoveryStatus) {
    for(const row of messageListRef.value?.querySelectorAll<HTMLElement>('[data-message-sequence]') ?? []) {
      const content=row.querySelector<HTMLElement>('.message-bubble') ?? row,rect=content.getBoundingClientRect()
      const top=Math.max(rect.top,viewport.top,0),bottom=Math.min(rect.bottom,viewport.bottom,innerHeight)
      const left=Math.max(rect.left,viewport.left,0),right=Math.min(rect.right,viewport.right,innerWidth)
...
watch(
  ...
  ([conversationId, conversationKind, messages, loading], previous) => {
    ...
    const messagesChanged = previous?.[2] !== messages || previous?.[4] !== props.recoveryTerminals
...
<template>
  <div ref="thread" class="message-thread" aria-live="polite">
    <div v-if="recoveryStatus" class="thread-state" role="status">
      <p>{{ recoveryStatus }}</p>
    </div>
    <div v-else-if="loading" class="thread-state">
      <span class="thread-spinner" />
      <p>正在载入对话…</p>
    </div>

    <div v-else-if="displayRows.length === 0" class="thread-state empty-thread">
    ...
    <ol v-else ref="messageList" class="message-list">
      ...
      <li v-if="row.terminal" class="terminal-row" :data-message-sequence="row.sequence">{{ terminalLabel(row.terminal.state) }}</li>
      <template v-else>
      <li
        v-for="message in row.message ? [row.message] : []"
...
<style scoped>
.terminal-row { padding: 12px 16px; text-align: center; color: var(--ink-soft); font-size: var(--font-caption); }
...
.sender-name { padding-left: 7px; font-size: var(--mx-typography-caption-size); }
.message-bubble { position: relative; min-width: 50px; border: 1px solid rgba(255,255,255,.7); }
.message-bubble--self { border-color: rgba(255,255,255,.26); color: var(--mx-color-message-on-own); }
.message-bubble--peer { color: var(--mx-color-message-on-peer); }
...
.message-text { margin: 0; font-size: var(--mx-typography-body-size); line-height: var(--mx-component-message-line-height); overflow-wrap: anywhere; white-space: pre-wrap; }
.message-placeholder { color: inherit; font-size: var(--mx-typography-caption-size); font-style: italic; opacity: .68; }
.reply-quote { display: grid; padding: 7px 9px; margin-bottom: 7px; gap: 2px; border-left: 3px solid currentColor; border-radius: 7px; font-size: var(--mx-typography-caption-size); background: rgba(16,35,63,.08); opacity: .78; }
...
.burn-label { display: inline-block; margin-top: 6px; padding: 3px 6px; border: 1px solid currentColor; border-radius: 7px; font-size: var(--mx-typography-micro-size); font-weight: 750; opacity: .68; }
.message-meta { display: flex; min-height: 15px; padding: 0 5px; align-items: center; gap: 6px; font-size: var(--mx-typography-micro-size); }
.delivery-failed { color: var(--mx-color-status-danger); font-weight: 650; }
.mention-read-status { min-height: 24px; padding: 0 3px; border: 0; color: var(--mx-color-action-text); font: inherit; font-weight: 700; background: transparent; cursor: pointer; }
.message-row--self .mention-read-status { color: color-mix(in srgb, var(--mx-color-action-text) 85%, var(--mx-color-text-secondary)); }
...
.message-actions button { padding: 1px 5px; border: 0; font-size: var(--mx-typography-micro-size); background: transparent; cursor: pointer; }
.thread-state { display: grid; height: 100%; min-height: 320px; place-items: center; align-content: center; color: var(--mx-color-text-secondary); text-align: center; }
.thread-state p { margin: 8px 0 0; font-size: var(--mx-typography-caption-size); }
.thread-spinner { width: 26px; height: 26px; border: 2px solid rgba(10,132,255,.16); border-top-color: var(--mx-color-action-text); border-radius: 50%; animation: spin .8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.empty-wave { display: grid; width: 66px; height: 66px; margin-bottom: 17px; place-items: center; border: 1px solid rgba(255,255,255,.75); color: var(--mx-color-action-text); font-size: 28px; }
.typing-pill { position: sticky; bottom: 4px; display: flex; width: max-content; padding: 8px 12px; margin: 10px auto 0; align-items: center; gap: var(--mx-spacing-1); border: 1px solid rgba(255,255,255,.76); border-radius: var(--mx-shape-radius-pill); font-size: var(--mx-typography-caption-size); backdrop-filter: blur(18px); }
.typing-pill span { width: 4px; height: 4px; border-radius: 50%; background: var(--mx-color-brand-blue); animation: bounce 1s infinite alternate; }
...
.message-thread { padding: 22px clamp(18px, 3vw, 38px); background: var(--mx-color-background-surface); }
.message-list { width: 100%; max-width: none; gap: var(--mx-spacing-3); }
.message-row { gap: 10px; }
.message-avatar { margin-bottom: 17px; }
.message-stack { gap: 3px; }
.sender-name { color: var(--mx-color-text-tertiary); font-weight: 500; }
.message-bubble {
  padding: 9px 13px;
  border: 0;
  border-radius: var(--mx-shape-radius-message) var(--mx-shape-radius-message) var(--mx-shape-radius-message) var(--mx-component-message-tail-radius);
  background: var(--mx-color-message-peer);
  box-shadow: none;
}
.message-bubble--self {
  border-radius: var(--mx-shape-radius-message) var(--mx-shape-radius-message) var(--mx-component-message-tail-radius) var(--mx-shape-radius-message);
  background: var(--mx-color-message-own);
  box-shadow: none;
}
.message-bubble--attachment { padding: 3px; }
.message-meta { color: var(--mx-color-text-tertiary); }
.message-actions button { color: var(--mx-color-text-secondary); }
.empty-wave {
  width: 58px;
  height: 58px;
  border: 0;
...
.typing-pill {
  border-color: var(--mx-color-border-glass);
  color: var(--mx-color-text-secondary);
  background: var(--mx-color-material-surface);
  box-shadow: 0 4px 16px var(--mx-color-shadow-default);
