...
import BroadcastRecipientList from './BroadcastRecipientList.vue'
import MobileBroadcastWorkspace from './MobileBroadcastWorkspace.vue'
...
async function selectBucket(bucket: string): Promise<void> {
  ...
  try {
    const result = await api.broadcasts.recipients(broadcast.value.id, bucket)
    if (generation === recipientGeneration) recipients.value = result
  } catch (cause) {
    if (generation === recipientGeneration) recipientError.value = cause instanceof Error ? cause.message : '成员明细加载失败'
  } finally {
    if (generation === recipientGeneration) recipientLoading.value = false
  }
}

const recipientError = shallowRef('')
let recipientGeneration = 0
watch(() => broadcast.value?.id, () => {
...
onBeforeUnmount(() => { recipientGeneration += 1 })
...
<template>
  <MobileBroadcastWorkspace
    ...
    @delete="emit('delete')"
  />
  <main v-else class="broadcast-workspace apple-content-surface" aria-live="polite">
    ...
    <section v-else-if="detail && broadcast" class="broadcast-document" :data-priority="broadcast.priority">
      ...
      <section v-if="receiver" class="receipt-panel" :class="{ 'receipt-panel--pending': completionPending }" aria-labelledby="receipt-title">
        ...
        <p v-if="receiver.confirmedAt" class="receipt-note">
          已于 {{ formatDateTime(receiver.confirmedAt) }} 提交；这条广播已在你的列表中标记为完成。
        </p>
      ...
      <section v-if="detail.createdByCurrentUser || statistics" class="statistics-panel" aria-labelledby="statistics-title">
        ...
        <div v-if="statistics" class="statistics-content">
          ...
          <BroadcastRecipientList v-if="selectedBucket" :recipients="recipients" :loading="recipientLoading" :error="recipientError" :can-remind="broadcast.status === 'ACTIVE'" @remind="emit('remind', $event)" />
...
.broadcast-document[data-priority="IMPORTANT"] .document-rail { background: var(--warning); }
...
.broadcast-back-button:hover { color: var(--accent-text); background: var(--active); }
...
.broadcast-document[data-priority="IMPORTANT"] .priority-mark { color: var(--warning); background: color-mix(in srgb, var(--warning) 10%, var(--fill)); }
.broadcast-document[data-priority="EMERGENCY"] .priority-mark { color: var(--danger); background: color-mix(in srgb, var(--coral) 10%, var(--fill)); }

.heading-copy { min-width: 0; flex: 1; }
.priority-kicker { margin: 1px 0 5px; color: var(--ink-soft); font-size: var(--font-micro); font-weight: 700; }
.broadcast-title { margin: 0; font-size: clamp(22px, 3vw, 30px); line-height: 1.2; letter-spacing: -0.045em; }
.document-actions { display: flex; flex: 0 0 auto; align-items: center; gap: var(--space-2); }
.status-badge { padding: 5px 9px; border-radius: var(--radius-pill); color: var(--success); font-size: var(--font-caption); font-weight: 700; background: color-mix(in srgb, var(--green) 10%, transparent); }
.status-badge--expired { color: var(--warning); background: color-mix(in srgb, var(--warning) 10%, transparent); }
.status-badge--cancelled { color: var(--ink-soft); background: var(--fill); }
.status-badge--completed { color: var(--success); background: color-mix(in srgb, var(--green) 12%, var(--fill)); }
.cancel-button { display: inline-flex; min-height: 34px; padding: 0 12px; align-items: center; justify-content: center; border: 0; border-radius: var(--radius-control); color: var(--danger); font-size: var(--font-caption); font-weight: 680; line-height: 1; white-space: nowrap; background: color-mix(in srgb, var(--danger-bg) 9%, var(--fill)); cursor: pointer; }
.cancel-button:hover:not(:disabled) { background: color-mix(in srgb, var(--danger-bg) 16%, var(--fill)); }
.cancel-button:disabled { cursor: wait; opacity: .55; }
.delete-button { display: inline-flex; min-height: 34px; padding: 0 12px; align-items: center; justify-content: center; border: 0; border-radius: var(--radius-control); color: var(--danger); font-size: var(--font-caption); font-weight: 680; line-height: 1; white-space: nowrap; background: color-mix(in srgb, var(--danger-bg) 16%, var(--fill)); cursor: pointer; }
.delete-button:disabled { cursor: wait; opacity: .55; }
.sender-value { display: grid; gap: 2px; }
.sender-value small { color: var(--ink-soft); font-size: var(--font-micro); }
...
.meta-label { margin-bottom: 4px; color: var(--ink-faint); font-size: var(--font-caption); }
.meta-value { margin: 0; overflow: hidden; font-size: var(--font-caption); font-weight: 600; text-overflow: ellipsis; white-space: nowrap; }

.broadcast-content { min-height: 150px; padding: 30px 4px 34px; }
.broadcast-content p { margin: 0; font-size: var(--font-body-lg); line-height: 1.85; white-space: pre-wrap; overflow-wrap: anywhere; }
.content-images { display: flex; flex-wrap: wrap; margin-top: 16px; gap: var(--space-2); }
.content-images img { width: min(170px, 100%); height: 120px; border-radius: var(--radius-control); object-fit: cover; }
.broadcast-content .content-location { margin-top: 14px; color: var(--ink-soft); font-size: var(--font-caption); }

.receipt-panel,
.statistics-panel { margin-top: 14px; padding: var(--space-5); border: 1px solid var(--separator); border-radius: 18px; background: var(--surface-tint); }
.receipt-panel--pending { border-color: color-mix(in srgb, var(--green) 18%, var(--separator)); }
.panel-heading { display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); }
.panel-actions { display: flex; min-width: 0; flex-wrap: wrap; align-items: center; justify-content: flex-end; gap: var(--space-2); }
.panel-kicker { margin: 0 0 4px; color: var(--ink-faint); font-size: var(--font-caption); font-weight: 700; }
.panel-title { margin: 0; font-size: 17px; letter-spacing: -0.025em; }
.receipt-state { display: grid; width: 34px; height: 34px; place-items: center; border-radius: 50%; color: var(--warning); background: color-mix(in srgb, var(--warning) 11%, var(--fill)); }
.receipt-state--handled { color: var(--success); background: color-mix(in srgb, var(--green) 11%, var(--fill)); }
.receipt-note { margin: 12px 0 0; color: var(--ink-soft); font-size: var(--font-caption); line-height: 1.6; }
.receipt-note--warning { color: var(--warning); }
.confirmation-actions { display: flex; margin-top: 15px; flex-wrap: wrap; gap: var(--space-2); }
.confirmation-button {
  display: inline-flex;
  min-height: 40px;
  padding: 0 14px;
...
.confirmation-button--selected { border-color: color-mix(in srgb, var(--green) 45%, transparent); color: var(--success); background: color-mix(in srgb, var(--success-bg) 9%, transparent); }
.confirmation-button--support:not(:disabled) { color: var(--danger); }
.confirmation-button:disabled { cursor: not-allowed; opacity: 0.58; }

.refresh-button--icon,
.toolbar-button { min-height: 36px; border: 1px solid var(--separator); border-radius: var(--radius-control); color: var(--ink-soft); font-size: var(--font-caption); font-weight: 650; line-height: 1; white-space: nowrap; background: var(--surface); cursor: pointer; }
...
.toolbar-button:hover:not(:disabled) { color: var(--accent-text); background: var(--active); }
.refresh-button--icon:disabled,
.toolbar-button:disabled { cursor: wait; opacity: 0.5; }
.statistics-content { margin-top: 17px; }
.progress-heading { display: flex; align-items: baseline; justify-content: space-between; gap: var(--space-3); }
.progress-heading strong { font-size: 26px; letter-spacing: -0.045em; }
.progress-heading span { color: var(--ink-soft); font-size: var(--font-micro); }
.progress-track { height: 7px; margin-top: 9px; border-radius: var(--radius-pill); background: var(--fill); overflow: hidden; }
.progress-value { display: block; height: 100%; border-radius: inherit; background: var(--green); }
.stat-grid { display: grid; margin-top: 17px; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: var(--space-2); }
.stat-item { display: grid; min-width: 0; padding: var(--space-3); gap: 3px; border: 0; border-radius: var(--radius-control); color: var(--ink); text-align: left; background: var(--surface); cursor: pointer; }
.stat-item:hover { background: var(--hover); }
.stat-item strong { font-size: var(--font-title-sm); letter-spacing: -0.03em; }
.stat-item span { color: var(--ink-faint); font-size: var(--font-caption); }
.response-breakdown { display: grid; margin-top: 13px; gap: 1px; border-radius: var(--radius-control); background: var(--separator); overflow: hidden; }
...
.breakdown-row span { color: var(--ink-soft); font-size: var(--font-micro); }
.breakdown-row strong { font-size: var(--font-caption); }
.unconfirmed-section { margin-top: 15px; }
.recipient-section { display: grid; margin-top: 15px; gap: 6px; }
.recipient-row { display: flex; min-height: 48px; padding: 7px; align-items: center; gap: 9px; border: 0; border-radius: var(--radius-control); color: var(--ink); text-align: left; background: var(--surface); cursor: pointer; }
...
.recipient-row small, .recipient-row em { color: var(--ink-soft); font-size: var(--font-micro); font-style: normal; }
.unconfirmed-label { color: var(--ink-soft); font-size: var(--font-micro); font-weight: 650; }
.user-id-list { display: flex; margin-top: 8px; flex-wrap: wrap; gap: 6px; }
.user-id { padding: 4px 7px; border-radius: var(--radius-sm); color: var(--ink-soft); font-family: var(--font-mono); font-size: var(--font-caption); background: var(--fill); }
.statistics-empty { display: grid; min-height: 90px; place-items: center; color: var(--ink-soft); font-size: var(--font-caption); }
...
.empty-symbol { display: grid; width: 64px; height: 64px; margin-bottom: 16px; place-items: center; border-radius: var(--radius-lg); color: var(--danger); background: color-mix(in srgb, var(--coral) 9%, var(--fill)); }
.empty-kicker { margin: 0 0 5px; color: var(--danger); font-size: var(--font-caption); font-weight: 700; }
.empty-title { margin: 0; font-size: var(--font-title); letter-spacing: -0.04em; }
.empty-copy { max-width: 290px; margin-top: 8px; color: var(--ink-soft); font-size: var(--font-caption); line-height: 1.6; }

.workspace-loading { display: grid; width: min(100%, 850px); margin: 0 auto; padding: 34px; gap: 14px; border: 1px solid var(--separator); border-radius: var(--radius-sheet); }
