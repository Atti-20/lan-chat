...
const completionHint = computed(() => {
  if (!needsEvidence.value) return '提交后将把这条广播标记为你已完成。'
  if (!ready.value) return '完成以上必填步骤后，才能提交任务完成结果。'
  return '所需材料已齐全，可以提交完成结果。'
})
  ...
  <section class="completion-panel" aria-labelledby="completion-title">
    ...
    <div class="completion-requirements">
      ...
      <div v-if="broadcast.requireImageProof" class="requirement-row" :class="{ 'requirement-row--ready': uploads.length > 0 }">
        ...
        <button class="evidence-button" type="button" :disabled="uploading || uploads.length >= 3 || submitting" @click="imageInput?.click()">
          {{ uploading ? '上传中…' : uploads.length ? '继续添加' : '上传图片' }}
        </button>
        <input ref="imageInput" hidden type="file" accept="image/*" multiple :disabled="uploading || uploads.length >= 3 || submitting" @change="upload">
      </div>

      <div v-if="uploads.length" class="upload-list">
        <span v-for="image in uploads" :key="image.id" class="upload-item">
          {{ image.originalName }}
          <button type="button" aria-label="移除图片" @click="uploads = uploads.filter((item) => item.id !== image.id)"><UiIcon name="close" :size="16" /></button>
        </span>
      </div>
...
<style scoped>
.completion-panel { display: grid; margin-top: 16px; padding: 15px; gap: var(--space-3); border: 1px solid color-mix(in srgb, var(--green) 24%, var(--separator)); border-radius: var(--radius-md); background: color-mix(in srgb, var(--green) 4%, var(--surface)); }
.completion-header { display: flex; align-items: center; gap: 10px; }
.completion-mark { display: inline-grid; width: 34px; height: 34px; flex: 0 0 auto; place-items: center; border-radius: var(--radius-control); color: var(--success); background: color-mix(in srgb, var(--green) 12%, var(--surface)); }
.completion-kicker { margin: 0 0 2px; color: var(--success); font-size: var(--font-caption); font-weight: 750; }
.completion-title { margin: 0; font-size: var(--font-body-lg); letter-spacing: -0.02em; }
.completion-requirements { display: grid; gap: 7px; }
.requirement-row { display: grid; min-height: 58px; padding: 9px 10px; grid-template-columns: 30px minmax(0, 1fr) auto; align-items: center; gap: 9px; border: 1px solid var(--separator); border-radius: var(--radius-control); background: var(--surface); }
...
.requirement-icon { display: inline-grid; width: 30px; height: 30px; place-items: center; border-radius: var(--radius-sm); color: var(--ink-soft); background: var(--fill); }
.requirement-row--ready .requirement-icon { color: var(--success); background: color-mix(in srgb, var(--green) 12%, var(--surface)); }
.requirement-copy { display: grid; min-width: 0; gap: 2px; }
.requirement-copy strong { font-size: var(--font-caption); }
...
.requirement-state { color: var(--success); font-size: var(--font-caption); font-weight: 700; }
.evidence-button { display: inline-flex; min-height: 32px; padding: 0 10px; align-items: center; justify-content: center; border: 1px solid var(--separator); border-radius: var(--radius-sm); color: var(--ink); font-size: var(--font-micro); font-weight: 650; line-height: 1; white-space: nowrap; background: var(--surface); cursor: pointer; }
.evidence-button:hover:not(:disabled) { border-color: color-mix(in srgb, var(--blue) 36%, var(--separator)); color: var(--accent-text); background: var(--active); }
...
.upload-item { display: inline-flex; max-width: 100%; padding: 4px 7px; align-items: center; gap: 5px; border-radius: 7px; color: var(--ink-soft); font-size: var(--font-micro); background: var(--fill); }
.upload-item button { display: inline-flex; width: 20px; height: 20px; padding: 0; align-items: center; justify-content: center; border: 0; border-radius: 6px; color: var(--danger); font-size: var(--font-subtitle); line-height: 1; background: transparent; cursor: pointer; }
.completion-error { margin: -2px 0 0; color: var(--danger); font-size: var(--font-micro); }
.complete-button { display: inline-flex; min-height: 48px; padding: 0 14px; align-items: center; justify-content: center; gap: 7px; border: 0; border-radius: var(--radius-control); color: white; font-size: var(--font-caption); font-weight: 700; line-height: 1; background: var(--success-bg); box-shadow: 0 5px 12px color-mix(in srgb, var(--green) 22%, transparent); cursor: pointer; }
.complete-button:hover:not(:disabled) { background: color-mix(in srgb, var(--success-bg) 86%, #000); }
