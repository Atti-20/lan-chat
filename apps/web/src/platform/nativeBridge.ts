import type { NativeBridge, RuntimeInfo, DesktopNode, DesktopNavigationTarget, RecoveryNotificationRequest } from '../../../../packages/platform-ports/src/index'
export type {
  DesktopPlatform, DesktopNodeSource, DesktopNodeHealth, DesktopNavigationKind,
  RuntimeInfo, DesktopNode, DesktopNavigationTarget, NotificationInput, UpdateResult,
  ConfirmOptions, NativeFileSaveResult, NativeFileSaveProgress, NativeFileSaveOptions, NativeBridge,
} from '../../../../packages/platform-ports/src/index'
import type { AuthSession, NodeRuntimeStatus } from '../types'
...
const recoveryAlerts = new Map<string,Notification>()
let recoveryNotificationOwner:string|null=null
async function applyRecoveryNotification(input:RecoveryNotificationRequest):Promise<void> {
  if(input.kind==='OWNER') recoveryNotificationOwner=input.owner
  if(recoveryNotificationOwner!==input.owner) throw new Error('STALE_NOTIFICATION_OWNER')
  if(isTauriRuntime()) {
    const {invoke}=await import('@tauri-apps/api/core')
    if(recoveryNotificationOwner!==input.owner) throw new Error('STALE_NOTIFICATION_OWNER')
    await invoke('recovery_notification',{input})
    return
  }
  if(isCapacitorRuntime()) throw new Error('RECOVERY_NOTIFICATION_REQUIRES_FLUTTER')
  if(input.kind==='OWNER' || input.kind==='CANCEL_ALL') {
    recoveryAlerts.forEach(alert=>alert.close());recoveryAlerts.clear();return
  }
  if(!input.id) throw new Error('INVALID_NOTIFICATION_ID')
  const notificationId=input.id
  if(input.kind==='CANCEL') {recoveryAlerts.get(input.id)?.close();recoveryAlerts.delete(input.id);return}
  if(!('Notification' in window) || Notification.permission!=='granted') throw new Error('NOTIFICATION_PERMISSION_REQUIRED')
  recoveryAlerts.get(input.id)?.close()
  const alert=new Notification('MeshX',{body:'你有新消息，打开 MeshX 查看',tag:input.id})
  recoveryAlerts.set(input.id,alert)
  alert.onclick=()=>{
    alert.close()
    if(recoveryNotificationOwner===input.owner && input.target) {
      window.focus()
      void import('./desktopNavigation').then(({handleNotificationNavigation}) => {
        if (recoveryNotificationOwner === input.owner) return handleNotificationNavigation(input.target!)
      })
    }
  }
  alert.onclose=()=>{if(recoveryAlerts.get(notificationId)===alert) recoveryAlerts.delete(notificationId)}
}
...
const webBridge: NativeBridge = {
  notificationRecovery: applyRecoveryNotification,
  runtime: () => 'web',
  ...
  notify: async ({ title, body, dedupeKey }) => {
    ...
    try {
      const alert=new Notification(title, { body })
      const key=`legacy:${dedupeKey || Date.now()}`
      recoveryAlerts.set(key,alert)
      alert.onclose=()=>{if(recoveryAlerts.get(key)===alert) recoveryAlerts.delete(key)}
    } catch (cause) {
  ...
  listenForNavigation: async (listener) => {
    ...
    const actionListener = await onAction((notification) => {
      ...
      void (async () => {
        ...
        try {
          listener({...JSON.parse(raw) as DesktopNavigationTarget,notification:true})
        } catch {
