...
export function claimNavigationForCurrentNode(
  ...
  if (claimed.notificationOwner
    && claimed.notificationOwner !== `${selectedNode()?.origin || window.location.origin}|${readSession()?.userId || ''}`) {
    return null
  }
...
async function handleTarget(rawTarget: DesktopNavigationTarget): Promise<void> {
  ...
  if (!target) return
  const targetKey = JSON.stringify([target.kind, target.value, target.nodeOrigin, target.notificationOwner, target.messageId])
  ...
  if (target.notificationOwner) {
    const session = readSession()
    if (!session?.token) {
      // Keep the body-free route until login; consumption rechecks node and owner.
      storePendingTarget(target)
      return
    }
    if (target.notificationOwner !== `${selectedNode()?.origin || window.location.origin}|${session.userId || ''}`) return
  }
  ...
  storePendingTarget(target)
  if (target.nodeOrigin && (selectedNode()?.origin || window.location.origin) !== target.nodeOrigin) {
...
export async function handleNotificationNavigation(target: DesktopNavigationTarget): Promise<void> {
  await handleTarget({...target,notification:true})
}
