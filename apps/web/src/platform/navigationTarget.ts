import type { NavigationKind, NavigationTarget } from '../../../../packages/platform-ports/src/runtime'
export type { NavigationKind, NavigationTarget } from '../../../../packages/platform-ports/src/runtime'
...
export function normalizeNavigationTarget(raw: unknown): NavigationTarget | null {
  ...
  if(target.messageId!==undefined && (typeof target.messageId!=='string' || !target.messageId || target.messageId.length>128)) return null
  if (target.notificationOwner !== undefined
    && (typeof target.notificationOwner !== 'string' || !target.notificationOwner || target.notificationOwner.length > 2048)) return null
  return { kind, value, nodeOrigin: origin, ...(target.messageId ? {messageId:target.messageId} : {}), ...(target.notification===true ? {notification:true} : {}), ...(typeof target.notificationOwner==='string' && target.notificationOwner.length<=2048 ? {notificationOwner:target.notificationOwner} : {}) }
}
...
export function parseNotificationNavigation(action: NotificationActionLike): NavigationDelivery | null {
  ...
  const actionId = typeof action.actionId === 'string' && action.actionId.trim()
    ...
    : 'tap'
  return { deliveryId: `${notificationId}:${actionId}`, target: {...target, notification:true} }
}
...
function sameTarget(first: NavigationTarget, second: NavigationTarget): boolean {
  return first.kind === second.kind
    ...
    && (first.messageId || null) === (second.messageId || null)
    && (first.notificationOwner || null) === (second.notificationOwner || null)
    && Boolean(first.notification) === Boolean(second.notification)
}
