...
const auth = useAuth()
const chat = useChat({
  enableRecovery: import.meta.env.VITE_MESHX_MUTATION_RECOVERY === 'true',
...
const conversationWritable = computed(() => {
  if (!recoveryCanSend.value) return false
...
const conversationStatusLabel = computed(() => {
  if(recoveryStatus.value) return recoveryStatus.value
  if(!recoveryCanSend.value) return '当前会话暂不可发送，历史消息仍可查看'
...
const connectionCopy = computed(() => recoveryStatus.value || (reconnecting.value
  ? '正在重连'
...
watch(chat.recoveryPhase,phase=>{if(phase==='ONLINE_SAFE'){const target=pendingDesktopNavigation();if(target) void openDesktopNavigation(target)}})

async function openDesktopNavigation(target: DesktopNavigationTarget): Promise<void> {
  if (loading.value || target.kind === 'node'
    || (target.notification && chat.usesMutationRecovery.value && chat.recoveryPhase.value !== 'ONLINE_SAFE')) return
  const claimedTarget = claimNavigationForCurrentNode(target)
  if (!claimedTarget) return
  target = claimedTarget
  if(target.notification && !chat.recoveryNotificationRouteAllowed(target.messageId,target.value)) {
    toast.push('这条通知对应的消息已不可用','warning');return
  }
