...
const RECOVERY_STORE = 'recoveryGeneration'
...
// The marker is read inside every legacy transaction, including queued writes.
// Aborting that same transaction prevents a late old writer from reviving data.
function legacyTransaction(database: IDBDatabase, stores: string | string[], mode: IDBTransactionMode): IDBTransaction {
  const names = typeof stores === 'string' ? [stores] : stores
  if (!names.some(name => [OUTBOX_STORE, MESSAGE_STORE, POSITION_STORE, DIRECTORY_STORE].includes(name))) {
    return database.transaction(stores, mode)
  }
  const transaction = database.transaction([...new Set([...names, RECOVERY_STORE])], mode)
  const marker = transaction.objectStore(RECOVERY_STORE).get('active')
  marker.onsuccess = () => { if (marker.result) transaction.abort() }
  return transaction
}
...
interface RecoveryStoredRecord {
  key: 'active'
  owner: string
...
function validateRecoveryRecord(value: RecoveryStoredRecord, owner: string): void {
  if (value.owner !== owner || value.recoveryStoreVersion !== 2 || !Number.isSafeInteger(value.revision)
    || value.revision < 1 || !value.snapshot || typeof value.snapshot !== 'object' || Array.isArray(value.snapshot)) {
    throw new Error('恢复缓存账号或版本无效')
  }
}

/** Loaded content is quarantined input; the coordinator must complete recovery before display. */
export async function loadRecoverySnapshot(owner: string): Promise<RecoveryLocalSnapshot | null> {
  const database = await openDatabase()
  const transaction = database.transaction(RECOVERY_STORE, 'readonly')
  const done = transactionDone(transaction)
  const value = await requestResult(transaction.objectStore(RECOVERY_STORE).get('active')) as RecoveryStoredRecord | undefined
  await done
  if (!value) return null
  validateRecoveryRecord(value, owner)
  return { revision: value.revision, snapshot: value.snapshot }
}

/** One transaction replaces body/proof/outbox data and removes the legacy copies. */
export async function commitRecoverySnapshot(owner: string, expectedRevision: number, snapshot: Record<string, unknown>): Promise<number> {
  if (!owner || !Number.isSafeInteger(expectedRevision) || expectedRevision < 0 || expectedRevision >= Number.MAX_SAFE_INTEGER) {
    throw new Error('恢复缓存版本无效')
  }
  const captured = storageSnapshot(snapshot)
  const database = await openDatabase()
  const stores = [RECOVERY_STORE, OUTBOX_STORE, MESSAGE_STORE, POSITION_STORE, DIRECTORY_STORE]
  ...
  try {
    ...
    const previous = await requestResult(store.get('active')) as RecoveryStoredRecord | undefined
    if (previous) validateRecoveryRecord(previous, owner)
    if ((previous?.revision ?? 0) !== expectedRevision) throw new Error('RECOVERY_GENERATION_CONFLICT')
    const next: RecoveryStoredRecord = { key: 'active', owner, recoveryStoreVersion: 2, revision: expectedRevision + 1, snapshot: captured }
    validateRecoveryRecord(next, owner)
    store.put(next)
    for (const name of stores) if (name !== RECOVERY_STORE) transaction.objectStore(name).clear()
    await done
...
export async function loadOutbox(): Promise<OutboxEntry[]> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, OUTBOX_STORE, 'readonly')
...
export async function saveOutboxEntry(entry: OutboxEntry): Promise<void> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, OUTBOX_STORE, 'readwrite')
...
export async function cacheMessages(messages: readonly ChatMessage[]): Promise<void> {
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, MESSAGE_STORE, 'readwrite')
...
export async function loadCachedMessages(
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, MESSAGE_STORE, 'readonly')
...
export async function savePosition(conversationId: string, sequence: number): Promise<void> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, POSITION_STORE, 'readwrite')
...
export async function loadPositions(): Promise<Record<string, number>> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, POSITION_STORE, 'readonly')
...
export async function saveConversationDirectory(
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, DIRECTORY_STORE, 'readwrite')
...
export async function loadConversationDirectory(): Promise<ConversationDirectoryRecord | null> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, DIRECTORY_STORE, 'readonly')
...
export async function loadCachedAvatarImage(cacheKey: string): Promise<CachedAvatarImage | null> {
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, AVATAR_IMAGE_STORE, 'readonly')
  const record = await requestResult(
...
export async function cacheAvatarImage(cacheKey: string, blob: Blob): Promise<void> {
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, AVATAR_IMAGE_STORE, 'readwrite')
...
export async function saveDirectFile(record: DirectFileRecord): Promise<void> {
  const database = await openDatabase()
  const transaction = legacyTransaction(database, DIRECT_FILE_STORE, 'readwrite')
...
export async function loadDirectFile(transferId: string): Promise<DirectFileRecord | null> {
  ...
  const database = await openDatabase()
  const transaction = legacyTransaction(database, DIRECT_FILE_STORE, 'readonly')
  const record = await requestResult(
...
export async function clearLocalChatDatabase(): Promise<void> {
  ...
  const transaction = database.transaction(
    [OUTBOX_STORE, MESSAGE_STORE, POSITION_STORE, DIRECTORY_STORE, DIRECT_FILE_STORE, AVATAR_IMAGE_STORE, RECOVERY_STORE],
    'readwrite',
  )
