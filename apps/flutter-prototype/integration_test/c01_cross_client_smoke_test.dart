  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await waitFor(
        () =>
            controller.online &&
            controller.conversations.any((item) => item.id == 'group:$groupId'),
      ...
      if (broadcastId > 0) {
        ...
        await waitFor(
          () =>
              find.byKey(const Key('complete-broadcast')).evaluate().isNotEmpty,
        ...
        if (expectContentImage) {
          ...
          final imageEntry = find.byKey(const Key('broadcast-content-image-0'));
        ...
        if (expectExpiry) {
          ...
          await waitFor(
            () =>
                find.byKey(const Key('complete-broadcast')).evaluate().isEmpty,
          ...
          try {
            final removed = await controller.api!.broadcastDetail(broadcastId);
          ...
          if (imageErrorFirst.isNotEmpty) {
            ...
            expect(platform.selectedFile, isNull);
            final rejected = await controller.api!.broadcastDetail(broadcastId);
          ...
          if (cancelImageFirst) {
            ...
            await tester.tap(find.byKey(const Key('complete-broadcast')));
            await waitFor(() => platform.busy, 'native picker must be active');
          ...
          final completed = await controller.api!.broadcastDetail(broadcastId);
