...
@CapacitorPlugin(
        ...
        permissions = @Permission(
                strings = { Manifest.permission.ACCESS_LOCAL_NETWORK },
                alias = "localNetwork"))
public class MeshXDiscoveryPlugin extends Plugin {
    private static final String TAG = "MeshXDiscovery";
    private static final String CONTROL_SERVICE_TYPE = "_meshx-control._tcp.";
    private static final String LEGACY_SERVICE_TYPE = "_lanchat._tcp.";
    ...
    public void scan(PluginCall call) {
        if (Build.VERSION.SDK_INT >= 37
                && getPermissionState("localNetwork") != PermissionState.GRANTED) {
            requestPermissionForAlias(
                    "localNetwork", call, "localNetworkPermissionCallback");
            return;
        }
        startScan(call, CONTROL_SERVICE_TYPE, true);
    }
    ...
    private void localNetworkPermissionCallback(PluginCall call) {
        if (getPermissionState("localNetwork") == PermissionState.GRANTED) {
            ...
            call.reject(
                    "未获得局域网访问权限；可在系统设置中允许后重试，或手动输入节点地址",
                    "LOCAL_NETWORK_PERMISSION_DENIED");
        }
    }

    private void startScan(PluginCall call, String serviceType, boolean allowLegacyFallback) {
        ...
        ScanSession session = new ScanSession(
                call, nsdManager, acquireMulticastLock(), serviceType, allowLegacyFallback);
        try {
            Log.i(TAG, "Starting DNS-SD scan for " + serviceType);
            nsdManager.discoverServices(serviceType, NsdManager.PROTOCOL_DNS_SD, session);
    ...
    private final class ScanSession implements NsdManager.DiscoveryListener {
        ...
        public void onServiceFound(NsdServiceInfo serviceInfo) {
            if (!serviceType.equalsIgnoreCase(serviceInfo.getServiceType())) return;
        ...
        private void finish() {
            ...
            if (origins.isEmpty() && allowLegacyFallback) {
                startScan(call, LEGACY_SERVICE_TYPE, false);
                return;
            }
