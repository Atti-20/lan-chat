...
public interface ChatMessageMapper extends BaseMapper<ChatMessage> {
    ...
    @Select("SELECT conversation_id FROM chat_message WHERE message_id = #{messageId} LIMIT 1")
    String selectConversationIdByMessageId(@Param("messageId") String messageId);

    /** Current read serializes terminal transitions, even after an earlier MVCC snapshot. */
    @Select("SELECT * FROM chat_message WHERE message_id = #{messageId} LIMIT 1 FOR UPDATE")
    ChatMessage selectByMessageIdForUpdate(@Param("messageId") String messageId);
