...
public interface TemporaryRoomMapper extends BaseMapper<TemporaryRoom> {
    ...
    @Select("SELECT id FROM temporary_room WHERE room_code = #{roomCode}")
    Long selectIdByRoomCode(@Param("roomCode") String roomCode);

    @Select("SELECT user_id FROM conversation_member WHERE conversation_id=CONCAT('temporary:',#{roomId}) AND left_time IS NULL ORDER BY user_id FOR UPDATE")
    List<Long> selectActiveMemberIdsForUpdate(@Param("roomId") Long roomId);

    @Select("SELECT role FROM conversation_member WHERE conversation_id=CONCAT('temporary:',#{roomId}) AND user_id=#{userId} AND left_time IS NULL LIMIT 1 FOR UPDATE")
    String selectMemberRoleForUpdate(@Param("roomId") Long roomId, @Param("userId") Long userId);
