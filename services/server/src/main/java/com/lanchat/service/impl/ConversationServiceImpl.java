...
public class ConversationServiceImpl implements ConversationService {
    ...
    private com.lanchat.recovery.ConversationWriteGuard writeGuard;

    @org.springframework.beans.factory.annotation.Autowired
    private com.lanchat.recovery.AccessMutationRecorder accessMutationRecorder;

    @Override
    public void lockConversationForWrite(String conversationId) { writeGuard.lock(conversationId); }

    @Override
    public void lockPrivateConversationForWrite(Long firstUserId, Long secondUserId) {
        String cid = ConversationIds.privateConversation(firstUserId, secondUserId);
        conversationMapper.insertIfAbsent(cid, "PRIVATE", null);
        writeGuard.lock(cid);
    }

    @Override
    public boolean canAccessForWrite(String conversationId, Long userId) {
        return writeGuard.permission(conversationId, userId).readAllowed();
    }

    @Override
    public List<Long> getReadableRecipientsForWrite(String conversationId) { return writeGuard.recipients(conversationId); }
    ...
    public String ensurePrivateConversation(Long firstUserId, Long secondUserId) {
        ...
        var newEntries = new java.util.TreeSet<Long>();
        if (accessMutationRecorder != null && accessMutationRecorder.enabled()) {
            for (Long user : java.util.List.of(firstUserId,secondUserId)) {
                if (!accessMutationRecorder.hasDirectoryEntry(conversationId,user)) newEntries.add(user);
            }
        }
        ...
        for (Long user : newEntries) accessMutationRecorder.directoryAdded(conversationId,user);
    ...
    public String ensureGroupConversation(Long groupId) {
        ...
        var membersQuery = new LambdaQueryWrapper<GroupMember>().eq(GroupMember::getGroupId, groupId);
        if (org.springframework.transaction.support.TransactionSynchronizationManager.isActualTransactionActive()) {
            membersQuery.orderByAsc(GroupMember::getUserId).last("FOR UPDATE");
        }
        List<GroupMember> members = groupMemberMapper.selectList(membersQuery);
    ...
    public String resolveForMessage(Long senderId,
        ...
        if (!hasPrivateTarget && !hasGroupTarget
            ...
            if (!writeGuard.permission(resolved, senderId).sendAllowed()) {
