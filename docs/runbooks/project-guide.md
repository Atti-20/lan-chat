...
> 本手册于 2026-09-08 从根 README 拆出，保留详细配置与历史能力说明。带日期的测试/设备结论属于当时证据；当前结构见 [架构地图](../../ARCHITECTURE.md)，新任务从模块指令开始。
...
V2.x 的需求到代码映射见 [实施状态-LAN-first-V2.0.md](../archive/local/PRD/v2/实施状态-LAN-first-V2.0.md)；V3.0 的精确完成边界、外部依赖和验收证据见 [实施状态-V3.0.md](../archive/local/PRD/v3/docs/v3/实施状态-V3.0.md)。
...
协议详情见 [可靠消息协议-V1.md](../archive/local/PRD/可靠消息协议-V1.md)。
...
| Android 端 | 目标 Flutter；现有旧壳为 Capacitor/WebView，最低 API 26，以工程配置为准 |
| iOS 端 | 目标 Flutter；现有旧壳为 Capacitor/WKWebView/SPM，最低 iOS 15，以工程配置为准 |
| 本地离线 | IndexedDB |
...
apps/android/                          复用 Vue UI 的 Capacitor Android 薄壳
  ...
  scripts/sync-web.mjs                 将 apps/web/dist-mobile 同步进 APK/AAB
...
Vite 构建产物写入 `services/server/src/main/resources/static/app/`，由 Spring Boot 同源托管。
...
```bash
npm run build:desktop --prefix apps/web
...
Web、Desktop 与 Android 的可见 UI 都由 `apps/web/` 提供；Android 工程只承载
...
正式工程位于 `apps/ios`，复用 `apps/web` 界面和 Keychain、Bonjour、系统文件桥接。
...
- 当前候选发布版本为 v0.3.0，以仓库根目录的 `VERSION` 为准；尚未创建通过全部发布门禁的正式版本。V3.0 P0 的文档保留为历史能力阶段说明，其代码边界和未完成验收见 [V3.0 实施状态](../archive/local/PRD/v3/docs/v3/实施状态-V3.0.md)。
- 《需求分析-LAN-first-V2.0》和《功能分析-LAN-first-V2.0》中的独立节点复制、冲突合并和性能目标不等于已经完成。
- 仓库内 [需求分析.md](../archive/local/PRD/需求分析.md) 与 [功能分析.md](../archive/local/PRD/功能分析.md) 是 V1.0 历史稿，仅用于版本对照。
